...
public class ReedSolomon {
	...
	/**
	 * Rows from the matrix for encoding parity, each one as its own byte array
	 * to allow for efficient access while encoding.
	 */
...
//		checkBuffersAndSizes(outputs, 0, byteCount, 1);
	...
	public void encodePartialParity(byte[][] shards, int offset, int byteCount, int parityNum) {
		...
		System.arraycopy(shards, dataShardCount, outputs, 0, parityNum);

		// Do the coding.
		codingLoop.codeSomeShards(parityRows, shards, dataShardCount, outputs, parityNum, offset, byteCount);
	}
	 ...
	 * @param byteCount
	 *            The number of bytes to check in each shard.
	 */
	 ...
	 * Returns true if the parity shards contain the right data.
	 *
	 * This method may be significantly faster than the one above that does not
	 ...
	 *            The number of bytes to check in each shard.
	 * @param tempBuffer
	 *            A temporary buffer (the same size as each of the shards) to
	 ...
	 *
	 * If any shards are missing (based on the flags in shardsPresent), the data
...
//		int shardsNum = shards.length;
	...
	/**
	 * Create the matrix to use for encoding, given the number of data shards
	 * and the number of total shards.
	 *
